	...
	return class BaseSheetSkyfall extends HandlebarsApplicationMixin(Base) {
		...
		async _onDrop(event) {
			...
			switch (type) {
				case "ActiveEffect":
					...
					break;
				case "Item":
					if (item.type == "sigil") return ui.notifications.warn("NOTIFICATIONS.SigilDropOnActor", { localize: true });
					if (item.type == "class") {
						const existingClass = this.document.items.find(i => i.system.identifier == item.system.identifier);
						console.log(existingClass);
						if (existingClass) {
							return existingClass.system.levelUp(this.document.system.level.value + 1);
						}
					}
					break;
				default:
			...
			if (this.document.documentName == "Actor" && item.type == "sigil") return;
		...
		static async #onCreate(event, target) {
			...
			if (create == "skill") {
				...
				} else if (key in this.actor.system.skills) return ui.notifications.warn("SKYFALL.ALERTS.DUPLICATESKILL", { localize: true });
		...
		static async #onEdit(event, target) {
			...
			if (fieldPath) {
				...
				if ("sheet" in datamodel) datamodel.sheet;
					...
					new skyfall.sheets.ActorConfigurationSheet({
						document: this.document,
						fieldPath: fieldPath,
						skyfallConfig: {
							schema: datafield,
		...
		static async #onItemRecharge(event, target) {
			...
			if (item.system.reload) {
				...
				if (!ammo) {
					return ui.notifications.error("NOTIFICATIONS.ConsumableNotFound", { localize: true });
				}
